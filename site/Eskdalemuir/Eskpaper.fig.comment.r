...
# produce the figures for the article:
...
#------------------------------------------------------------------
# To run from the R prompt type: source("Eskpaper.fig.comment.r")
# input: the file ESKDALEMUIR_T+06
# output: the images Eskpaper.Fig1.ps, Eskpaper.Fig2.ps, 
# Eskpaper.Fig3.ps, Eskpaper.Fig4.ps, Eskpaper.Fig5.ps
#------------------------------------------------------------------
# Note: the input file ESKDALEMUIR_T+06 and the source code
# Eskpaper.fig.comment.r should be in the same directory from 
...
postscript("Eskpaper.Fig1.ps",horizontal=FALSE)
	par(mfrow=c(2,1))
	par(pty='m')
	par(oma=c(6,2,6,2))
	...
	axis(2)
	box()
	mtext("Eskdalemuir observations",side=3,line=2,cex=2)
	...
	axis(2)
	box()
	mtext("Eskdalemuir T+6 forecasts",side=3,line=2,cex=2)
	mtext("precipitation (mm)",side=2,line=3,cex=1.5)
	mtext("time (year)",side=1,line=3,cex=1.5)
dev.off()
...
postscript("Eskpaper.Fig2.ps",horizontal=FALSE)
	par(pty='s')
	par(mfrow=c(2,1))
...
postscript("Eskpaper.Fig3.ps",horizontal=FALSE)
	par(pty='s')
	par(mfrow=c(2,2))
	par(oma=c(6,2,6,2))
...
# hit rate
	...
	plot(quantile(Obs.d,ths),H,type="l",lwd=3,xlab="threshold (mm)",ylab="hit rate (H)",ylim=c(0,0.8))
...
# false alarm rate
	...
	plot(quantile(Obs.d,ths),F,type="l",lwd=3,xlab="threshold (mm)",ylab="false alarm rate (F)",ylim=c(0,0.54))
	lines(quantile(Obs.d,ths),Fp,lty=2,lwd=3)
	lines(quantile(Obs.d,ths),1-ths,lty=3,lwd=3)
	legend(x=20,y=0.5,legend=c("Met Office T+6", "Persistence","Random (F=p)"),lty=c(1,2,3),lwd=3,cex=1,xjust=1)
	axis(3,at=quantile(Obs.d,ths),labels=round(ths,3))
	mtext("1 - base rate",side=3,line=3,cex=0.8)
...
# ROC curve on log axes
	...
	plot(log(F/(1-F)),log(H/(1-H)),xlab="log F/(1-F)",ylab="log H/(1-H)",ylim=c(-7,2),xlim=c(-7,2),type="l",lwd=3)
...
postscript("Eskpaper.Fig4.ps",horizontal=FALSE)
	par(pty='m')
	par(mfrow=c(2,2))
	par(oma=c(6,2,6,2))
...
# PC
	...
	axis(3,at=quantile(Obs.d,ths),labels=round(ths,3))
	mtext("1 - base rate",side=3,line=3,cex=0.8)
	legend(x=20,y=0.5,legend=c("Met Office T+6", "Persistence","Random (F=p)"),lty=c(1,2,3),lwd=3,cex=1,xjust=1,yjust=0)
...
# ETS
	plot(quantile(Obs.d,ths),ETS,type="l",lwd=3,xlab="threshold (mm)",ylab="Equitable Threat Score (ETS)",ylim=c(0,max(ETS)))
...
# log odds ratio
	...
	plot(quantile(Obs.d,ths),log(OR),type="l",lwd=3,xlab="threshold (mm)",ylab="log odds ratio",ylim=c(0,max(log(OR))))
...
postscript("Eskpaper.Fig5.ps",horizontal=FALSE)
	par(pty='m')
	par(mfrow=c(1,1))
	par(oma=c(6,2,6,2))
	...
	lines(quantile(Obs.d,ths),rep(0,length(ths)),lty=3,lwd=3)
	axis(3,at=quantile(Obs.d,ths),labels=round(ths,3))
	mtext("1 - base rate",side=3,line=3,cex=1)
dev.off()
